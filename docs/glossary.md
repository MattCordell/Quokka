...
### Extrapolation
The normalisation "annual" applies so it is always a true 365-day figure regardless of the loaded span (which is **provider-dependent** — weeks to ~2 years). If the candidate window (most recent 365 calendar days, or the whole span if shorter) has full **Data Coverage**, no scaling is applied. Otherwise, scaling is computed from *actual coverage*, per dimension, not one blanket factor: each Usage Category's kWh scales by `ANNUAL_DAYS / daysWithData[category]` (a solar Generation register reading every day must not inherit a General meter's gap, or vice versa), and each TOU band's kWh scales by a per-day-of-week factor comparing how many times that specific day of the week was actually sampled against how many times it *truly occurs* in a real 365-day calendar year (six days at 52 occurrences, one at 53 — never the fractional average `365/7`, which would silently inflate a genuinely gapless full year). Labelled an *estimate*, with a **seasonal-bias** warning and, when any day of the week has zero samples, every time-of-use plan is excluded from the ranked comparison entirely rather than shown at an understated, not-comparable-to-flat total ([ADR-0006](adr/0006-annual-extrapolation.md)).

### Data Coverage
Per **Usage Category**, the count of distinct in-period days with *any* reading at all on that category's mapped registers, regardless of quality flag — a coverage count, not a quality count. Not the same thing as the **non-actual day count** below: a day can have full coverage (a reading exists) yet still be non-actual (the reading is an estimate/substitute), and a day can be missing entirely (no coverage) without ever being flagged non-actual. Kept per-category, not unioned, so a fully-covered register can't mask a gap in a different one. Compared against **Days in Period** to disclose a shortfall, and against 365 to decide **Extrapolation**.

### Bill
The computed result for one **Plan** over one **Billing Period**: `Supply + Usage + Controlled Load − Solar Credit`, then **Discount** applied. All rates GST-inclusive. Components computed at full precision; **only the two totals are rounded** to the cent (independently, from the full-precision pre-discount base — never one total minus a rounded discount), and a **net-credit total may be negative** ([ADR-0004](adr/0004-rounding-and-bill-sign.md)). A Bill also carries a **non-actual day count**: the number of distinct in-period days where any counted register resolved to a non-actual (estimated/substituted) quality flag, deduped across registers ([ADR-0003](adr/0003-quality-flag-handling.md)); and, when priced under **Extrapolation**, the General category's scaling factor and sampled-day count, so a projected figure can always be told apart from a measured one — this describes the category-level projection only, not necessarily the (different, per-day-of-week) factor actually applied to a TOU bill's bands.
